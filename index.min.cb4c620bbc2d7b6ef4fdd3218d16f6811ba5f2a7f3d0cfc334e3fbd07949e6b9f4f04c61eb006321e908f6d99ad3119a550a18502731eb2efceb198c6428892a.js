...
`}),e.add({id:5,href:"/docs/exploit/stackoverflow/ret2libc/",title:"ret2libc",description:`0x01 介绍 # 我们一般gcc编译不加 -static 的话默认就是动态链接，动态链接就是不将代码嵌入到你的程序中，而是等程序运行时再链接到动态链接库，从而减小程序的大小。我们可以对比以下动态和静态的区别
...
printf(\u0026quot;%d, %d, %d\\n\u0026quot;, 1, 2, 3); 生成的汇编为
804841c: 6a 03 push 0x3 804841e: 6a 02 push 0x2 8048420: 6a 01 push 0x1 8048422: 68 c4 84 04 08 push 0x80484c4 8048427: e8 b4 fe ff ff call 80482e0 \u0026lt;printf@plt\u0026gt; 执行printf时的栈结构为`,content:`0x01 介绍 # 我们一般gcc编译不加 -static 的话默认就是动态链接，动态链接就是不将代码嵌入到你的程序中，而是等程序运行时再链接到动态链接库，从而减小程序的大小。我们可以对比以下动态和静态的区别
#include\u0026lt;stdio.h\u0026gt; int main() { puts(\u0026quot;hello world\\n\u0026quot;); return 0; } // 动态链接: gcc test.c -o test // 静态链接: gcc test.c -o test_static -static 静态链接的话会把很多我们用不到的函数也给添加到程序中
静态编译的程序在内存中的布局为除了程序本身并没有其他的东西
而动态链接的话只有两个动态链接库的函数存留（而且保留的不是函数的代码，而是函数的符号）
在内存中的布局多了一个libc即动态链接库和ld链接器
...
具体的可以自己多调试看看
`}),e.add({id:6,href:"/docs/exploit/",title:"Exploit",description:"",content:""}),e.add({id:7,href:"/docs/exploit/formatstring/%E6%A0%BC%E5%BC%8F%E5%8C%96%E5%AD%97%E7%AC%A6%E4%B8%B2%E6%BC%8F%E6%B4%9E/",title:"格式化字符串漏洞",description:`0x01 介绍 # 在学习C语言的时候，一般第一个学习的函数就是printf函数。它最常用的用法是通过格式化字符串来格式化输出内容，例如
...
printf(\u0026quot;%-2d\u0026quot;, 1); // 输出 \u0026quot;1 \u0026quot; +：输出数值类型时始终显示正负号`,content:`0x01 介绍 # 在学习C语言的时候，一般第一个学习的函数就是printf函数。它最常用的用法是通过格式化字符串来格式化输出内容，例如
printf(\u0026quot;%s\u0026quot;, buf); 但是也可以简单的写为
printf(buf); 这样也可以将buf字符串内容打印出来。但是这样做有个很大的风险，printf默认它的第一个参数中如果有格式化控制符则会进行解析，如果你对buf字符串输入了格式化控制符的话，这里就会出错，这个就是格式化字符串漏洞。
当我们往buf里输入格式化控制符的话，它会去找对应的参数进行解析，这里所谓的参数就是给printf的第一个之后的参数。但是我们简写的例子并没有给它第二个参数，它就会按照调用约定，自动认为后一个值为它的参数，然后我们可以利用格式化控制符对其进行操作。
在此之前我们需要详细的学习一下格式化控制符以及修饰符。
...
printf(\u0026quot;%2$d\u0026quot;, 1, 2, 3); // 输出 \u0026quot;2\u0026quot; 学过了这些基本知识再来看看格式化字符串漏洞的两种表现形式：任意读和任意写。任意读比较好理解，我们一开始的那个例子就是任意读的应用，可以读取栈中的内容，如果我们加上参数控制符，那么就可以任意读取栈中你想要的数据
...
`}),e.add({id:8,href:"/docs/",title:"Docs",description:"learn docs",content:""}),search.addEventListener("input",t,!0);function t(){const s=5;var n=this.value,o=e.search(n,{limit:s,enrich:!0});const t=new Map;for(const e of o.flatMap(e=>e.result)){if(t.has(e.doc.href))continue;t.set(e.doc.href,e.doc)}if(suggestions.innerHTML="",suggestions.classList.remove("d-none"),t.size===0&&n){const e=document.createElement("div");e.innerHTML=`No results for "<strong>${n}</strong>"`,e.classList.add("suggestion__no-results"),suggestions.appendChild(e);return}for(const[r,a]of t){const n=document.createElement("div");suggestions.appendChild(n);const e=document.createElement("a");e.href=r,n.appendChild(e);const o=document.createElement("span");o.textContent=a.title,o.classList.add("suggestion__title"),e.appendChild(o);const i=document.createElement("span");if(i.textContent=a.description,i.classList.add("suggestion__description"),e.appendChild(i),suggestions.appendChild(n),suggestions.childElementCount==s)break}}})()